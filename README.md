...
| Restent identifiés mais non corrigés (moindre priorité, cf. échange) : styles `:hover` du menu latéral non limités aux appareils à pointeur réel (`(hover: hover)`, pertinent pour une tablette tactile assez large), version de `@supabase/supabase-js` figée uniquement en version majeure sur le CDN, absence d'`aria-label` sur les boutons à icône seule, pas de prise en compte de `prefers-reduced-motion` | ⏳ différé |
