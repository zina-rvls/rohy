...
| Animation d'accueil juste après connexion (login/inscription) | ✅ fait — écran de lancement animé (motif tissé, bandes qui glissent une à une, aux couleurs de la variante multicolore de la marque), affiché à chaque démarrage de l'app pendant ~5s. Le wordmark "Rohy" et la baseline apparaissent en fondu juste après la fin du tissage, le temps d'être lus avant que l'écran ne disparaisse. La connexion/le chargement des données continuent en parallèle pendant ce délai (pas d'attente supplémentaire une fois l'écran retiré) |
| Le menu du bas se retrouvait parfois caché derrière la barre du navigateur mobile lui-même (barre d'outils ancrée en bas, barre de navigation gestuelle Android, certains navigateurs intégrés) | ✅ fait — `100dvh` seul ne retire pas toujours correctement cette zone de l'écran selon le navigateur/contexte. Ajout d'un ajustement via `window.visualViewport` (qui reflète la zone réellement visible et se met à jour à chaque affichage/masquage de ce chrome), posé en variable CSS (`--app-vh`) utilisée en complément de `100dvh`. Vérifié en simulant un rétrécissement du viewport visible (équivalent à une barre d'outils qui apparaît) : le menu du bas suit bien la nouvelle hauteur réelle plutôt que de rester sur une valeur obsolète |
...
| Pas de page "À propos" dans l'app | ✅ fait — nouvel écran (logo multicolore + wordmark, description de l'app), accessible depuis "Mon compte" → "À propos". Utilise la navigation standard de l'app (bouton retour, pile de navigation) plutôt qu'une fenêtre modale, pour un vrai écran à part entière |
...
| Paiement mobile money intégré (MVola / Orange Money / Airtel Money), avec mise à jour automatique du solde une fois le règlement effectué | ⏳ différé — pas juste un ajout de code : chaque opérateur exige un accord marchand (souvent via une entreprise enregistrée, avec frais), ou en passant par un agrégateur ayant déjà ces accords (ex. Manda, PaidMada, Efaina, PayBriq, Papi, Voaray — solutions locales à Madagascar avec API unifiée MVola/Orange Money/Airtel Money). Une fois l'accès obtenu, ça change le profil de risque de l'app (webhooks à sécuriser, idempotence, réconciliation, paiements partiels/échoués). Piste intermédiaire envisageable sans dépendance externe : enrichir le formulaire de règlement actuel avec le choix du moyen utilisé + un champ pour la référence de transaction reçue de l'opérateur (toujours une confirmation manuelle, mais avec une vraie trace) |
| Sur l'écran de lancement, les bandes glissaient en ligne droite jusqu'à leur place — un simple mouvement de translation, sans rappeler le tissage du motif final | ✅ fait — chaque bande garde sa trajectoire principale (bandes verticales tombent, horizontales glissent depuis le côté) mais ondule désormais aussi perpendiculairement à cette trajectoire (léger va-et-vient), avec une amplitude qui décroît jusqu'à l'arrivée exacte à sa position finale. Évoque un fil qui serpente en passant tour à tour au-dessus puis en dessous des autres bandes, plutôt qu'une translation rectiligne. Vérifié image par image (Playwright) que chaque bande atterrit bien exactement à sa place — le motif final reste identique au logo statique |
