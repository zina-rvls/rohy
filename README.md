...
| Sur mobile, fond noir visible autour/derrière l'app, et menu du bas qui ne reste pas au bord réel de l'écran | ✅ fait — cause racine commune : `.app-frame` avait une hauteur fixe (840px, pensée comme une "carte" desktop) qui ne correspond à aucun vrai viewport mobile. En dessous de 640px de large, la carte occupe désormais tout l'écran (100dvh, avec repli 100vh) ; au-delà, l'aspect carte centrée est conservé à l'identique. `data-theme` est aussi désormais porté par `<html>` (pas seulement `.app-frame`), pour que le fond de `<body>` corresponde toujours au thème actif |
