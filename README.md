...
| Le problème persistait quand même sur mobile (Chrome iOS, donc moteur WebKit comme Safari) alors qu'il était corrigé partout ailleurs | ✅ fait — cause probable : `transform-box: fill-box` + `transform-origin: center`, utilisés pour faire grandir chaque bande depuis son propre centre, ont un support inégal sur WebKit pour les éléments `<path>` (le point de départ de l'échelle peut y être mal calculé) — ce qui peut décaler la bordure par rapport au remplissage pendant la transition et recréer un bug d'apparence proche de celui de la transparence, sans qu'il soit reproductible dans l'environnement de vérification (Chromium). Remplacé par un point d'origine fixé en coordonnées explicites par bande (calculées à la main, en style inline), qui contourne entièrement ce calcul de boîte englobante et est fiable sur tous les navigateurs, y compris les anciennes versions de WebKit |
